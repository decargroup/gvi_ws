...
class GVI:
        ...
        for x_k in self.factored_states:
            ...
            if isinstance(x_k, ProcessFactor):
                if k == 0:
                    ...
                    self.information[k : k + dof, k : k + dof] = np.copy(
                        x_k.information
                    )
                    k += dof
                else:
                    ...
                    self.information[k : k + state_dof, k : k + state_dof] = np.copy(
                        x_k.get_information()
                    )
                    # Cross Information Terms
                    self.information[k - state_dof : k, k : k + state_dof] = np.copy(
                        x_k.information[0:state_dof, state_dof:]
                    )
                    self.information[k : k + state_dof, k - state_dof : k] = np.copy(
                        x_k.information[state_dof:, 0:state_dof]
                    )
                    k += state_dof
    ...
    def solve(self):
        ...
        while True:
            ...
            self.new_information = self._force_sparsity(
                self.new_information, deg=self.factor_dof
            )
            ...
            if self.new_cost >= self.cur_cost:
                if self.backtrack_on:
                    ...
                    backtrack_success = self.backtrack(
                        delta_mu, delta_info, max_iters=15, alpha=self.init_alpha
                    )
        ...
        for x_k in self.factored_states:
            ...
            if stamp not in stamp_list:
                ...
                if (
                    not np.allclose(
                        mean.reshape((-1, 1)),
                        est_list[idx].state.value.reshape((-1, 1)),
                    )
                ) or (not np.allclose(covar, est_list[idx].covariance)):
    ...
    def from_map(self, map_covariance):
        ...
        self.information = self._force_sparsity(
            force_PSD(scipy.linalg.inv(np.copy(map_covariance))), self.factor_dof
        )
        # Update states accordingly
        phi = np.zeros((1, 1))
        for x_k in self.factored_states:
    ...
    def _force_sparsity(self, info_matrix: np.ndarray, deg: int):
        sparse_matrix = np.triu(
            np.tril(info_matrix.copy(), k=(2 * deg - 1)), k=-(2 * deg - 1)
        )
