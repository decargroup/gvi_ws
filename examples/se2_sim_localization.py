...
if __name__ == "__main__":
    ...
    # Actual noise values that were used in generation
    real_meas_noise = noise_config["MEAS_NOISE"]
    real_proc_noise = noise_config["PROC_NOISE"]

    # Load data
    DATA_PATH = f"./data/sim/meas_data_se2_sim_{real_meas_noise}.pkl"
    ...
    if MAP_LOSS_FUN == "cauchy":
        ...
        c_pos = fitted_noise_dict["Asymmetric Cauchy"][1]
        c_neg = fitted_noise_dict["Asymmetric Cauchy"][2]
        MAP_LOSS_FUN = AsymmetricCauchyLoss()
    else:
    ...
    elif GVI_LOSS_FUN == "cauchy":
        GVI_LOSS_FUN = gvi.graph.losses.CauchyLoss()
    else:
    ...
    if config["USE_MAP"]:
        ...
        if config["EXPORT"]:
            path = f"./data/results/se2_sim/{real_meas_noise}"
