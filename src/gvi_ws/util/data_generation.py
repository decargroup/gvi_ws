...
def randvec(
    cov: np.ndarray, num_samples: int = 1, method: str = "gaussian"
) -> np.ndarray:
    """
...
class DataGenerator:
    ...
    process_noise_type : str, optional
        Type of process noise to be added. If None, assumed to be Gaussian. Other types could be \"cauchy\" or \"student_t\".
    measurement_noise_type : str, optional
        Type of measurement noise to be added. If None, assumed to be Gaussian. Other types could be \"cauchy\" or \"student_t\".
    ...
    def __init__(
        ...
        input_freq: float,
        meas_model_list: List[MeasurementModel] = None,
        meas_freq_list: Union[float, List[float]] = None,
        meas_offset_list: Union[float, List[float]] = None,
        process_noise_type: str = "gaussian",
        measurement_noise_type: str = "gaussian",
    ):
    ...
    def generate(self, x0: State, start: float, stop: float, noise=False):
        ...
        for k in range(0, len(times) - 1):
            ...
            if not meas_generated:
                while times[k + 1] > meas.stamp and not meas_generated:
                    ...
                    meas.value = generate_measurement(
                        ...
                        model=meas.model,
                        noise=noise,
                        noise_type=self.measurement_noise_type,
                    ).value
            ...
            if noise:
                u = u.plus(randvec(Q, method=self.process_noise_type))
...
def generate_measurement(
    ...
    for x in x_list:
        ...
        og_shape = y.shape
        if noise:
            y = y.reshape((-1, 1)) + randvec(R, method=noise_type)
