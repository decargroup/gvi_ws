...
if __name__ == "__main__":
    ...
    fig_map, ax_map = nav.plot_nees(
        results=results_map, confidence_interval=0.975, label="MAP"
    )
    fig, ax = nav.plot_nees(
        results=results_gvi, ax=ax_map, confidence_interval=0.975, label="ESGVI"
    )
    ax.set_xlabel("Time (s)")
    ax.set_title("NEES")
    plt.savefig(
        f"/home/astirl/Documents/courses/assignments/mech_642/gvi_ws/figs/se2_aNEES.pdf"
    )
    plt.show()
