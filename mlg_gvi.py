...
from util.psd import force_PSD, force_sym, regularize, force_sym_PSD
...
class GVI:
    def __init__(
        self,
        factored_states: List[FactoredState],
        total_dim: int,
        backtrack_on=True,
        debug=False,
        max_iters=10,
        ...
        # Params
        # self.factored_states: List[FactoredState] = [
        #     fac_k.copy() for fac_k in factored_states
        # ]
        self.factored_states: List[FactoredState] = factored_states
        self.total_dim = total_dim
        self.total_landmark_dim = 0
        self.total_states_dim = total_dim
        self.debug = debug
        self.backtrack_on = backtrack_on
        self.backtrack_iters = backtrack_iters
        self.max_iters = max_iters
        self.state_dof = factored_states[0].dof
        ...
        for x_k in self.factored_states:
            ...
            if isinstance(x_k, PriorFactor):
                ...
                self.information[k : k + x_k.dof, k : k + x_k.dof] = (
                    x_k.get_information()
                )
                k += x_k.dof
            ...
            if isinstance(x_k, ProcessFactor):
                ...
                self.information[k : k + dof, k - dof : k] = (
        ...
        self.covariance = force_sym_PSD(scipy.linalg.inv(self.information))
    ...
    def solve(self):
        ...
        while True:
            ...
            if size_mu < 1e-8 and size_info < 1e-6:
                print("--------------------------------")
                print(
                    f"Iter: {n_iters} || Cost: {self.cur_cost} || Step size (mu): {size_mu} || Step size (info): {size_info}"
                )
            ...
            if np.abs(delta_cost / self.cur_cost) < 1e-6:
                ...
                print(f"|  No Cost Change: Converged in {n_iters} iterations!  |")
                print("--------------------------------")
                for x_k in self.factored_states:
                    x_k.update_factor(
                        total_mean=self.mean,
            ...
            if self.new_cost >= self.cur_cost:
                if self.backtrack_on:
                    ...
                    backtrack_success = self.backtrack(
                        np.copy(delta_mu),
                        np.copy(delta_info),
                        max_iters=self.backtrack_iters,
                    ...
                    print(
                        f"Exiting, didn't reduce cost from {self.new_cost} to {self.cur_cost}"
                    )
                    for x_k in self.factored_states:
                        x_k.update_factor(
                            total_mean=self.mean,
    ...
    def backtrack(
        self, delta_mu: np.ndarray, delta_info: np.ndarray, max_iters=10, alpha=1.0
    ):
        ...
        while True:
            ...
            proposed_info = force_sym_PSD(proposed_info)
            ...
            proposed_covar = force_sym_PSD(scipy.linalg.inv(proposed_info))
            ...
            if backtrack_iters % 5 == 0:
                print(
                    f"Backtrack #{backtrack_iters} || Alpha: {alpha} || Cost: {self.new_cost}"
                )
            ...
            if self.new_cost < self.cur_cost:
                ...
                self.new_mean, self.new_information, self.new_covariance = (
    ...
    def get_estimate_list(self, get_landmark=False):
        ...
        for x_k in self.factored_states:
            ...
            if stamp is None:
                ...
                if (not np.allclose(state_k.value, est_list[idx].state.value)) or (
                    not np.allclose(covar, est_list[idx].covariance)
                ):
    ...
    def from_map(self, map_covariance):
        ...
        self.information = self._force_sparsity(
            force_PSD(scipy.linalg.inv(np.copy(map_covariance))), self.state_dof
        )
        # Update states accordingly
        phi = np.zeros((1, 1))
        for x_k in self.factored_states:
    ...
    def _force_sparsity(self, info_matrix: np.ndarray):
        ...
        info_matrix[0 : self.total_states_dim, 0 : self.total_states_dim] = (
            sparse_matrix
        )
        return info_matrix
