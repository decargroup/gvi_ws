...
from util.psd import force_PSD, force_sym, force_sym_PSD
...
class FactoredState:
    def __init__(
        ...
        stamp: float,
        related_factor: Optional["FactoredState"] = None,
        cubature_type="GH",
        gh_degree=3,
    ):
        self.mean = mean.copy()
        ...
        if related_factor is not None:
            ...
            self.total_projection = np.vstack(
                (related_factor.projection, self.projection)
            )
        else:
        ...
        self.cubature_type = cubature_type
    ...
    def generate_unit_sigma_pts(self, cubature_type, order=None):
        if cubature_type == "GH":
            ...
            self._unit_sigma_pts, self.weights = self.cubature(
                order_p=self._gh_degree, state_dof=self.total_dof
            )
        elif cubature_type == "spherical":
            self.cubature: Callable = spherical_cubature
            self._gh_degree = order
            self._unit_sigma_pts, self.weights = self.cubature(
                order_p=None, state_dof=self.total_dof
            )
        else:
            raise NotImplementedError("Implement other cubature methods")

    def generate_new_sigma_pts(self):
        self.total_covariance = force_sym_PSD(self.total_covariance)
        self.total_information = force_sym_PSD(self.total_information)
        self.covariance = force_sym_PSD(self.covariance)
    ...
    def compute_expectations(self):
        ...
        if self.related_factor is not None:
            ...
            for i, w in enumerate(self.weights):
                ...
                phi_k_l = self.eval_phi(
                    cur_sigma_point=cur_sp, prev_sigma_point=prev_sp
                )
                expect_phi += w * phi_k_l
                prev_diff = prev_sp.minus(self.related_factor.mean).reshape(
                    (self.related_factor.dof, 1)
                )
    ...
    @abstractmethod
    def update_factor(
        self, total_mean, total_information, total_covariance, delta_mean=None
    ):
        pass
    ...
    def update_state(
        ...
        self.information = (
            self.projection @ np.copy(total_information) @ self.projection.T
        )
        self.covariance = (
            self.projection @ np.copy(total_covariance) @ self.projection.T
        )
        ...
        if self.group is not None:
            if delta_mean is None:
                ...
                self.mean = self.mean.plus(delta_mean_vector)
                if self.mean.direction == "left":
                    jac = self.group.left_jacobian(delta_mean_vector)
                    jac_inv = self.group.left_jacobian_inv(delta_mean_vector)
                elif self.mean.direction == "right":
                    jac = self.group.right_jacobian(delta_mean_vector)
                    jac_inv = self.group.right_jacobian_inv(delta_mean_vector)
...
class ProcessFactor(FactoredState):
    ...
    def update_factor(
        ...
        self.total_information = (
            self.total_projection @ np.copy(total_information) @ self.total_projection.T
        )
        self.total_covariance = (
            self.total_projection @ np.copy(total_covariance) @ self.total_projection.T
        )
        # Fix factor covar/info during retraction
        if (delta_mean is not None) and (self.group is not None):
            self.mean: MatrixLieGroupState
            delta_mean_vec: np.ndarray = self.total_projection @ delta_mean
            mean_prev = delta_mean_vec[0 : self.related_factor.dof]
            mean_cur = delta_mean_vec[self.related_factor.dof :]
            if self.mean.direction == "left":
    ...
    def set_cross_information(self, cross_info: np.ndarray):
        ...
        self.total_information[0 : self.related_factor.dof, self.dof :] = np.copy(
            cross_info
        )
        self.total_information[self.dof :, 0 : self.related_factor.dof] = np.copy(
            cross_info.T
        )
        return
    ...
    def copy(self):
        return ProcessFactor(
            ...
            related_factor=self.related_factor.copy(),
            process_model=self.process_model,
            proj_matrix=self.projection.copy(),
            u=self.u,
            stamp=self.stamp,
            cubature_type=self.cubature_type,
            gh_degree=self._gh_degree,
        )
...
class PriorFactor(FactoredState):
    ...
    def update_factor(
        ...
        return super().update_state(
            total_mean, total_information, total_covariance, delta_mean
        )
...
class LandmarkPriorFactor(PriorFactor):
    def __init__(
        ...
        super().__init__(
            mean, covariance, proj_matrix, prior, stamp, cubature_type, gh_degree
        )
...
class MeasurementSLAMFactor(MeasurementFactor):
    ...
    def eval_phi(self, cur_sigma_point: State, prev_sigma_point: State):
        comp_state = CompositeState(
            [cur_sigma_point, prev_sigma_point], stamp=self.stamp
        )
...
def construct_factor_list(
    ...
    prior_factor = PriorFactor(
        mean=x0.state.copy(),
        covariance=x0.covariance,
        proj_matrix=proj_0,
        prior=x0.copy(),
        stamp=x0.state.stamp,
        gh_degree=gh_deg,
        cubature_type=cubature_type,
    )
    ...
    for i in range(len(input_data) - 1):
        ...
        proc_factor_k = ProcessFactor(
            mean=x_k,
            covariance=P_k,
            proj_matrix=proj_k,
            related_factor=factored_state_list[-1],
    ...
    for i in range(len(meas_data)):
        ...
        if meas.stamp <= input_data[-1].stamp:
            ...
            meas_factor = MeasurementFactor(
                mean=fac_k.get_mean(),
                covariance=fac_k.get_covariance(),
                stamp=meas.stamp,
                proj_matrix=proj_k,
                meas=meas,
                gh_degree=gh_deg,
...
def construct_slam_factor_list(
    x0: StateWithCovariance,
    input_data: List[Input],
    meas_data: List[Measurement],
    landmark_data: List[StateWithCovariance],
    proc_model: ProcessModel,
    ...
    for l in landmark_data:
        ...
        landmark_factor = LandmarkPriorFactor(
            mean=l.state.copy(),
            covariance=l.covariance.copy(),
            proj_matrix=proj_0_landmark,
            prior=l.copy(),
            stamp=None,
            use_prior=use_prior,
            cubature_type=cubature_type,
    ...
    for i in range(len(meas_data)):
        ...
        if meas.stamp <= input_data[-1].stamp:
            ...
            meas.model = meas_model(
                fac_k.mean.state_id, landmark_id, meas.model.covariance(None)
            )
            landmark_factor = factored_landmark_dict.get(landmark_id)
            meas_factor = MeasurementSLAMFactor(
                mean=fac_k.get_mean(),
                covariance=fac_k.get_covariance(),
                proj_matrix=proj_k,
                meas=meas,
                stamp=meas.stamp,
                related_factor=landmark_factor,
                cubature_type=cubature_type,
                gh_degree=gh_deg,
            )
            factored_state_list.append(meas_factor)
...
def factor_list_from_map(
    opt_variables,
    problem: Problem,
    input_data: List[nav.types.Input],
    meas_data: List[Measurement],
    proc_model: ProcessModel,
    cubature_type="GH",
    gh_deg=3,
    ...
    prior_factor = PriorFactor(
        ...
        covariance=P0,
        proj_matrix=proj_0,
        prior=StateWithCovariance(state=x0_state.copy(), covariance=P0),
        stamp=x0_state.stamp,
        cubature_type=cubature_type,
        gh_degree=gh_deg,
    )
    factored_state_list.append(prior_factor)
...
def slam_factors_from_map(
    opt_variables,
    problem: Problem,
    input_data: List[nav.types.Input],
    meas_data: List[Measurement],
    landmark_data: List[VectorState],
    proc_model: ProcessModel,
    meas_model: MeasurementModel,
    use_prior=True,
    cubature_type="GH",
    ...
    for i in range(len(meas_data)):
        meas: Measurement = meas_data[i]
        if meas.stamp <= input_data[-1].stamp:
            fac_k_idx = find_nearest_stamp_idx(factored_stamp_list, meas.stamp)
            fac_k: FactoredState = factored_state_list[fac_k_idx]
            proj_k = np.copy(fac_k.projection)
            landmark_id = meas.model._landmark_state_id
            meas.model = meas_model(
            ...
            meas_factor = MeasurementSLAMFactor(
                ...
                stamp=meas.stamp,
                meas=meas,
                related_factor=related_landmark,
                cubature_type=cubature_type,
                gh_degree=gh_deg,
            )
            factored_state_list.append(meas_factor)
